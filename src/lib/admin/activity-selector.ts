...
export async function resolveAdminActivitySelector(
    selector: ActivitySelector,
    status: 'active' | 'draft' | 'archived' | 'all' = 'active',
): Promise<ActivityRow[]> {
    let query = supabaseServer.from('activities').select(SELECT);
    if (status === 'active') query = query.eq('is_active', true).eq('publication_status', 'approved').is('archived_at', null);
    if (status === 'draft') query = query.eq('publication_status', 'draft').is('archived_at', null);
    if (status === 'archived') query = query.eq('publication_status', 'archived');
