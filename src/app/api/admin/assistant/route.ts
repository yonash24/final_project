...
export async function POST(request: NextRequest) {
    ...
    if (body.action === 'confirm') {
        ...
        const { data: pending } = await supabaseServer.from('activity_change_requests').select('*')
            .eq('nonce_hash', hashNonce(token)).eq('actor_email', auth.profile.email).eq('status', 'pending').maybeSingle();
        if (!pending || new Date(pending.expires_at) <= new Date()) {
            if (pending) await supabaseServer.from('activity_change_requests').update({ status: 'expired' }).eq('id', pending.id);
        ...
        if (pending.operation === 'create') {
            ...
            if (error) return NextResponse.json({ error: 'לא ניתן ליצור את החוג.' }, { status: 500 });
            result = data;
        } else {
            ...
            if (error || !data) return NextResponse.json({ error: 'הפעולה נכשלה או שהחוג השתנה.' }, { status: 409 });
            result = data;
        }
    ...
    if (command.operation === 'query') return NextResponse.json(await getChatResponse(command.query || message));
    const permission = requirePermission(auth.profile, 'content:write');
    if (permission) return permission;
    ...
    if (command.operation !== 'create') {
        ...
        const matches = await getActivitiesByName(command.target_name);
